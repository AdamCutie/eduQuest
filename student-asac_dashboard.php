...
<head>
  <meta charset="UTF-8">
  <title><?= htmlspecialchars(ucfirst($type)) ?> Dashboard</title>
  ...
  <div class="assignment-box">
    <div class="left-section">
      ...
      <div class="assignment-header">
        <p><strong><?= htmlspecialchars(ucfirst($type)) ?>: <?= htmlspecialchars(ucfirst($title)) ?></strong></p>
        <p>Due Date: <?= htmlspecialchars(ucfirst($content['deadline'])) ?></p>
        <p>Score: <?= htmlspecialchars($content['st_score']) ?> / <?= htmlspecialchars($content['score']) ?></p>
      </div>
    ...
    <div class="right-panel">
      <form action="st_submission.php" method="POST" enctype="multipart/form-data">
        <button class="attachc-btn">Attach File</button>
        <input type="text" name="content_id" value="<?= htmlspecialchars($content_id) ?>" hidden>
        <input type="text" name="user_id" value="<?= htmlspecialchars($user_id) ?>" hidden>
        <input type="text" name="class_id" value="<?= htmlspecialchars($class_id) ?>" hidden>
        <input type="file" name="attachment" id="file-upload" class="upload-btn">
